...
def writeForwardRules(p4_info_helper,swIngress, swEgress,     //编写转发规则
                     dstEthernetAddr, dstIPAddr, dstPort):
    //swIngress:入口交换机    swEgress:出口交换机    dstEthernetAddr:目标以太网地址
    //dstIPAddr:目标IP地址    dstPort:目的端口
...
def issueswid(p4info_helper, swIngress, swEgress, swid):  //分配交换机id
    //swIngress:入口交换机    swEgress:出口交换机    swid：交换机id
    ...
    swIngress.WriteTableEntry(table_entry)
    print("Swid has been set to %s" % swIngress.name)
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        issueswid(p4info_helper, s1, s1, 1)
        issueswid(p4info_helper, s2, s2, 2)
        issueswid(p4info_helper, s3, s3, 3)
