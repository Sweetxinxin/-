...
header ipv4_t {
    bit<4>    version;
    bit<4>    ihl;
    bit<6>    diffserv;  //将 tos 拆分为两个字段 6 位 diffserv 和 2 位 ecn
    bit<2>    ecn;
    bit<16>   totalLen;
...
control MyIngress(inout headers hdr,
   ...
   apply {
        if (hdr.ipv4.isValid()) {   //ipv4有效
            if (hdr.ipv4.protocol == IP_PROTOCOLS_UDP) {   //协议为UDP
                expedited_forwarding();
	    }
            else if (hdr.ipv4.protocol == IP_PROTOCOLS_TCP) {  //协议为TCP
	        voice_admit();
            }
            ipv4_lpm.apply();
        }
