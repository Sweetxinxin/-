...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
    ...
    安装三条规则：
    1） ipv4_lpm表中入口交换机上的隧道入口规则
    将流量封装到具有指定ID的隧道中
    2） 入口交换机上的传输规则，根据指定的ID
    3） 出口开关上的隧道出口规则，用于解除交通堵塞
...
def readTableRules(p4info_helper, sw):
    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            table_name = p4info_helper.get_tables_name(entry.table_id)  //获取入口表id转为表名 
            print('%s: ' % table_name, end=' ')
            for s in entry.match:   //匹配 
                print(p4info_helper.get_match_field_name(table_name, s.field_id), end=' ')
                print('%r' % (p4info_helper.get_match_field_value(s),), end=' ')
            action = entry.action.action
            action_name = p4info_helper.get_actions_name(action.action_id)
            print('->', action_name, end=' ')
            for t in action.params:  //动作参数 
                print(p4info_helper.get_action_param_name(action_name, t.param_id), end=' ')
                print('%r' % t.value, end=' ')
            print()
